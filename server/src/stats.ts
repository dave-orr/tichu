...
/**
 * Remember who each signed-in player has shared a table with (feeds the invite
 * list). This is the only per-round write to user docs: all stats shown to
 * players are derived from the game history (see shared/src/stats.ts).
 */
export async function recordPlayedWith(room: Room): Promise<void> {
  if (!firebaseAdmin) return;
  const db = firebaseAdmin.firestore();
  const uidMap = buildUidMap(room);
  if (uidMap.size < 2) return;

  const batch = db.batch();
  for (const uid of uidMap.keys()) {
    const otherUids = [...uidMap.keys()].filter(u => u !== uid);
    // set+merge so a never-loaded user doc is created instead of failing the batch.
    batch.set(db.collection('users').doc(uid), { playedWith: arrayUnion(...otherUids) }, { merge: true });
  }
  await batch.commit();
}
